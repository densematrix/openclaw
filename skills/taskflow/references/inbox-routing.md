...
Use `skills/taskflow/examples/inbox-triage.lobster` through the managed Lobster run/approval/resume procedure in the parent TaskFlow skill. The default batch is synthetic and already classified. The workflow routes all items into `business`, `personal` and `later` ID lists, suspends for actual approval, then returns those lists without sending anything.
...
| ---------- | ----------------------------------------------------------------------------------------------------------------- |
| `business` | Post through an authorized channel adapter, persist the returned thread ID, then wait for a correlated human reply. |
